...
async function writeCollegeToFile(college){
    try {
        
        let college_data 
        let data = await readFromDisk()
        if(data == ""){
            college_data = []
        }
        
        else {
            college_data = JSON.parse(data)
        }
        
        college_data.push(college)
        
        await writeToDisk(JSON.stringify(college_data))
        return "SUCCESS"
...
function generateRandomCollegeData(name){

    let courses = [ 
        
        ["Computer Science","Mechanical Engineering","Electrical Engineering","BioTechnology"],
        ["Computer Science","Civil Engineering","Electronics Engineering","BioTechnology"],
        ["Computer Science","Information Technology","Nursing","Medical"],
        ["Food Technology","Civil Engineering","Nursing","Medical","Information Technology"],
    
    ]


    let random_course_choice = Math.floor(Math.random()*4)
    let random_year_founded = 1920 + (Math.round(Math.random()*90))
    let random_address = generateRandomAddress()

    let college = {
        name : name,
        year_founded:random_year_founded,
        country:"India",
        state:random_address.state,
        city:random_address.city,
        no_of_students:100,
        courses:courses[random_course_choice]
    }
...
async function saveCollegeData(i){
   if(i==100){
       return
   }
    ...
    let name ="college_"+i
    let college = generateRandomCollegeData(name)
    let result = await writeCollegeToFile(college)
    saveCollegeData(i+1)
   }
}


// saveCollegeData(0).catch(err => console.log(err))



async function generateRandomStudentData(college_name,i){
    if(i==100)
    return
    else {
        ...
        let skills = [
            ["C","Java","Python",".NET"],
            ["Java","C#","JavaScript"],
            ["Digital Electronics","Analog Electronics","DBMS"],
            ["Electron","JavaScript","ReactJS","Angular"]
        ]


        let random_index = (Math.floor(Math.random()*skills.length))
        let random_skill = skills[random_index]

        let random_batch = 1990+(Math.floor(Math.random()*30))
        let student = {
            name : faker.name.findName(),
            year_of_batch : random_batch,
            college_id : college_name,
            skills : random_skill
        }

        await writeCollegeToFile(student)

        await generateRandomStudentData(college_name,i+1)
...
async function main(i){
    if(i==100){
        ...
        let result = await generateRandomStudentData(`college_${i}`,0)
        await main(i+1)
    }
}

//main(0).catch(err => console.log(err))

function generateUniqueCourses(){
    let courses = [ 
    ...
    for(let i =0 ;i<courses.length ;i++){
        for(let j=0;j<courses[i].length ;j++){
            if(! unique_courses.includes(courses[i][j])){
                unique_courses.push(courses[i][j])
            }
        }
    }
